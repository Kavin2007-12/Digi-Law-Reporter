...
import { Eye, Edit3, Globe, Trash2, CheckCircle2, AlertTriangle } from 'lucide-react';
import { MOCK_CASES } from '../../data/adminMockData';

export default function AdminPublishedCases() {
  const navigate = useNavigate();
  const [casesList, setCasesList] = useState(MOCK_CASES.filter(c => c.status === 'Published'));
  const [deleteModalCase, setDeleteModalCase] = useState(null);

  const handleUnpublishCase = (caseId) => {
                ...
                casesList.map((item, idx) => (
                  <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="py-3.5 px-4 text-slate-500 text-xs whitespace-nowrap">
                      {item.judgmentDate || item.lastUpdated}
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-right whitespace-nowrap font-bold text-xs">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <span className="text-slate-300">/</span>
                        <button
                          onClick={() => handleUnpublishCase(item.id)}
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-xl p-6 max-w-md w-full border border-slate-200 shadow-xl space-y-4">
            <div className="flex items-center gap-2 text-red-600 font-bold">
              ...
              <span>Delete Published Case</span>
            </div>

            <p className="text-slate-600 text-xs leading-relaxed">
              Are you sure you want to delete published case <strong className="text-slate-900 font-bold">{deleteModalCase.caseNumber} ({deleteModalCase.title})</strong>?
            </p>
